...
Здесь самое важное наблюдение: на `Earnings22` качество резко падает. Это и есть сильный domain shift. Модель обучалась на LibriSpeech, а финансовая речь заметно отличается по лексике и условиям записи.
...
Greedy снова полностью плоский, а вот `beam + LM` при росте температуры только ухудшается. То есть в этой задаче temperature scaling проблему не решает.
...
Это, на мой взгляд, самый важный результат всей работы. Financial LM заметно улучшила именно `Earnings22`, но не помогла на `LibriSpeech`. Значит, проблема действительно была в домене, а не в том, что декодер работает неправильно.
...
Если говорить честно, главный итог этой домашки для меня такой: хорошая языковая модель помогает только тогда, когда она действительно подходит к домену.

## 10. Что не сделано

Отдельный прогон с `4-gram LM` из `Task 5` я в этой версии не делал.
